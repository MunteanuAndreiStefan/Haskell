...
Pana in acest moment, am realizat un evalpuator pentru expresii
...
In continuare vom scrie o functie pentru evalpuarea unei expresii
...
functia de evalpuare primeste doi parametri:

1) expresia aritmetica care trebuie evalpuata;
...
evalpuati in ghci expresiile aritmetice "x * (2 + y)", "(x + 10) * 2" si "(x1 + x2) * x" in
...
validateMaybeInt :: (Int -> Int -> Int) -> (Maybe Int -> Maybe Int -> Maybe Int)
validateMaybeInt funtionToApply _ Nothing = Nothing
validateMaybeInt funtionToApply Nothing _ = Nothing
validateMaybeInt funtionToApply (Just x)(Just y) = Just (funtionToApply x y)

-- sumaMaybe ::Maybe Int -> Maybe Int -> Maybe Int
sumaMaybe = (validateMaybeInt (+))
produsMaybe = (validateMaybeInt(*))

evalp :: Exp -> ValuationPar -> Maybe Int
evalp (Numar x) _ = Just x
evalp (Var x) val = val x
