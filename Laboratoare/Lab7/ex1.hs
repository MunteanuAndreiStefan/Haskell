...
executaInstr valuatie (Atrib x exp1) = Just (update valuatie x (evalp exp1 valuatie))


executaInstr valuatie (Cond exp1 program1 program2) = case (evalp exp1 valuatie) of
                                            Nothing   -> Nothing
                                            Just 0    -> executaProgram valuatie program2
                                            otherwise -> executaProgram valuatie program1
											
											
executaInstr valuatie (While exp1 program) = case (evalp exp1 valuatie) of
                                         Nothing -> Nothing
                                         Just 0 -> Just valuatie
                                         otherwise -> case (executaProgram valuatie program) of
                                                      Nothing -> Nothing
                                                      Just valuatie2 -> executaInstr valuatie2 (While exp1 program)

                                                      
executaInstr valuatie (For exp1 exp2 exp3 program) = case (evalp exp1 valuatie) of
                                        Nothing -> Nothing
                                        Just 0 -> Just valuatie
                                        otherwise -> case (evalp exp2 valuatie) of
                                                    Nothing -> Nothing
                                                    otherwise -> case (executaProgram valuatie program) of
                                                                    Nothing -> Nothing
                                                                    sumaMaybe (evalp exp1) (evalp exp3)
                                                                    Just valuatie2-> executaInstr valuatie2 (For exp1 exp2 exp2 program)
...
prog2 = [Atrib "n" (Numar 10),
        Atrib "s" (Numar 0),
        While (Var "n") [Atrib "s" (Suma (Var "s") (Var "n")),
...
-}
prog3 :: Program
prog3 = [Atrib "n" (Numar 10),
         Atrib "s" (Numar 0),
         Atrib "i" (Numar 0),
         For ((Var "i"),(Var "n"),(Atrib "n" (Diferenta (Var "n") (Numar 1)))) [Atrib "s" (Suma (Var "s") (Var "n"))],
         Atrib "result" (Var "s")]
{-
